chrome.storage.sync.get(
    ...
    async function(data) {
      if (data.isValidKey === true && data.enabledScripts.mek) {
        const userBilling = data.billingProfiles.find(profile => profile.billingId === data.selectedBillingProfile);
        const userSettings = data.userSettings; 
        await sleep(350);
        console.log("Clicking mek Purchase Button");
        click("sqs-block-button-element--small sqs-block-button-element")
        click("sqs-block-button-element--small")
        await sleep(500);
        click("Purchase");
        click("purchase");
        function sleep(ms) {
