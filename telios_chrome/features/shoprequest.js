...
chrome.storage.sync.get('profiles', function (items) {
    ...
    chrome.storage.sync.get('profileIndex', function (index) {
        ...
        const opts = {
            ...
            "checkout[email]": chosenProfile.email,
            "checkout[buyer_accepts_marketing]": "0",
            "checkout[buyer_accepts_marketing]": "1",
            "checkout[shipping_address][first_name]": chosenProfile.firstName,
            "checkout[shipping_address][last_name]": chosenProfile.lastName,
            "checkout[shipping_address][company]": "",
            "checkout[shipping_address][address1]": chosenProfile.address,
            "checkout[shipping_address][city]": chosenProfile.city,
            "checkout[shipping_address][country]": chosenProfile.country,
            "checkout[shipping_address][province]": chosenProfile.state,
            "checkout[shipping_address][zip]": chosenProfile.zipcode,
            "checkout[shipping_address][phone]": chosenProfile.phone,
            "checkout[shipping_address][first_name]": chosenProfile.firstName,
            "checkout[shipping_address][last_name]": chosenProfile.lastName,
            "checkout[shipping_address][company]": "",
            "checkout[shipping_address][address1]": chosenProfile.address,
        ...
        function submitPayment(url, profile, authenticityToken) {
            ...
            const opts = {
                ...
                "checkout[buyer_accepts_marketing]": "0",
                "checkout[buyer_accepts_marketing]": "1",
                "checkout[shipping_address][number]": chosenProfile.card.cardNumber,
                "checkout[shipping_address][name]": chosenProfile.firstName + chosenProfile.lastName,
                "checkout[shipping_address][expiry]": chosenProfile.card.month + chosenProfile.card.year,
                "checkout[shipping_address][verification_value]": chosenProfile.card.cvv,
                "checkout[client_details][browser_width]": "1903",
                "checkout[client_details][browser_height]": "969",
                "checkout[client_details][javascript_enabled]": "1",
                "checkout[client_details][color_depth]": "24",
        ...
        return fetch(
            `https://${siteUrl}/cart/shipping_rates.json?shipping_address[zip]=${
                ...
                chosenProfile.countryCode
            )}&shipping_address[province]=${encodeURIComponent(
                chosenProfile.state.toLowerCase()
            )}`,
            {
            method: "GET",
            headers: {
        ...
        skipToCheckoutFunc( 'radareatsshopifybitch')
